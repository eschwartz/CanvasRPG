...
var World = (function() {
	var worldClass = Class.extend({	
		...
		init: function(options) {
			...
			this.startPosition = this.startPositions || { x: this.width / 2, y: this.height / 2 };
			...
			// Move world on click
			this.stage.canvas.onclick = this.centerMap;
		},
		...
		centerMap: function(coord) {
			...
			if(!x || !y) { throw new Error("Unable to center map: invalid coordinates"); }
