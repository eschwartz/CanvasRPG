...
createjs.World = (function() {
	...
	var worldClass = Class.extend({	
		...
		isInViewport: function(x, y) {
			...
			isInView = {
				x: (coord.x >= -this.container.x) && (coord.x <= -this.container.x + this.stage.canvas.width),
				y: (coord.y >= -this.container.y) && (coord.y <= -this.container.y + this.stage.canvas.height)
			}
		...
		centerMap: function(coord) {
			var x, y;
			var target;
			x = (coord instanceof MouseEvent)? coord.offsetX: coord.x + 2*this.container.x;
			y = (coord instanceof MouseEvent)? coord.offsetY: coord.y + 2*this.container.y;
