...
createjs.World = (function() {
	...
	var worldClass = Class.extend({	
		...
		centerMap: function(coord) {
			var x, y;
			var target;
			x = (coord instanceof MouseEvent)? coord.offsetX: coord.x + 2*this.displayObject.x;
			y = (coord instanceof MouseEvent)? coord.offsetY: coord.y + 2*this.displayObject.y;

			if(!_.isNumber(x) || !_.isNumber(y)) { throw new Error("Unable to center map: invalid coordinates"); }
			
			target = {
				x: this.displayObject.x + (this.stage.canvas.width / 2) - x,
				y: this.displayObject.y + (this.stage.canvas.height / 2) - y,
			}
